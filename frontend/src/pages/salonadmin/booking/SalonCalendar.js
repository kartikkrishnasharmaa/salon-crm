  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        <button
          onClick={() => setViewMode(viewMode === "resources" ? "staff" : "resources")}
          style={{ 
            padding: '6px 12px',
            background: '#4e73df',
            color: '#fff',
...
const SalonCalendar = () => {
  ...
  const calculateCurrentTimePosition = () => {
    ...
    const slotHeight = timeSlots[0].getBoundingClientRect().height;
    const totalSlots = timeSlots.length;
    const minutesPerSlot = (22 - 8) * 60 / totalSlots; // 8am to 10pm
   
    const currentMinutes = hours * 60 + minutes;
    const startMinutes = 8 * 60; // 8am
    const slotIndex = Math.floor((currentMinutes - startMinutes) / minutesPerSlot);
   
    const headerHeight = calendarElement.querySelector('.rbc-time-header')?.getBoundingClientRect().height || 0;
    const position = headerHeight + (slotIndex * slotHeight) + (slotHeight / 2);
  ...
  return (
    <div>
      ...
      <div style={{ position: "relative", padding: "20px", textAlign: "center" }}>
        ...
        >
          {currentTime && <TimeIndicator currentTime={currentTime} />}
          ...
          <Calendar
            ...
            step={15} 
            timeslots={1}  // This sets how many time slots to show per step (1 slot per 30 minutes)
            showMultiDayTimes
